...
import type { Chapter, ChapterFormData, ChapterWithProgress } from '../types';
...
export const chaptersService = {
  ...
  async getBySubjectWithProgress(subjectId: string): Promise<ChapterWithProgress[]> {
    const { data, error } = await supabase
      .from('chapters')
      .select(`
        *,
    ...
    return (data || []).map(chapter => {
      ...
      const completedTopics = topics.filter((topic: any) => topic.is_completed).length;
  ...
  async getByIdWithProgress(id: string): Promise<ChapterWithProgress | null> {
    const { data, error } = await supabase
      .from('chapters')
      .select(`
        *,
  ...
  async create(subjectId: string, form: ChapterFormData, orderIndex: number): Promise<Chapter> {
    const payload = {
  ...
  async update(id: string, form: Partial<ChapterFormData>): Promise<Chapter> {
  ...
  async updateOrder(id: string, orderIndex: number): Promise<Chapter> {
    const { data, error } = await supabase
      .from('chapters')
